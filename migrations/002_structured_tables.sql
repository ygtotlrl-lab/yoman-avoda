-- ============================================================================
-- 002_structured_tables.sql — יומן העבודה עובר לטבלאות מובנות
-- ============================================================================
--
...
-- פעיל שנבדק בפועל אין נקודת חזרה. סדר ההרצה: שלב א ← אימות ש-
...
-- ב-localStorage. ⛔ אין לערבב בין שתי השכבות.
--
-- 2. **`client_id` נגזר ממפתח המיזוג, ואינו uuid חדש.**
-- ⛔ וזה **הפוך** מ-schar-limud בכוונה — שם `id` הוא `SERIAL`
...
-- שמנוע המיזוג משתמש בה. uuid שני ועצמאי היה מאפשר לאותה
...
-- ⛔ ובמיוחד: **אין `BEFORE UPDATE` שדורס את החותמת** — טריגר
...
-- ⛔ `SELECT, INSERT, UPDATE` ותו לא (כלל ברזל 10 סעיף 9) — מחיקה
