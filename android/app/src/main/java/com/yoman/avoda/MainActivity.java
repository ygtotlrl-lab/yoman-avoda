 ...
 *
 * <p>⚠️ And the share bridge — present because this app's config declares
 * {@code android.share}: the page produces an image, and the shell writes it to
 * cache, exposes it through FileProvider and fires ACTION_SEND.
 * ⛔ An app that does not declare it carries no bridge, no FileProvider and no
 * androidx dependency.
 */
public class MainActivity extends ShellActivity {
    ...
    //      dropped.
    //
    // The page uses whichever exists.
    @Override
    ...
    // ⛔ אין כאן יעד — הבורר של המערכת הוא שבוחר: ⚠️ יעד קבוע היה שולח את הדוח לאפליקציה אחת בלבד.
    private void shareImage(final String base64Data, final String mimeType) {
        try {
            byte[] bytes = Base64.decode(base64Data, Base64.DEFAULT);
            File dir = new File(getCacheDir(), "shared");
            dir.mkdirs();
            final File f = new File(dir, "ya_" + System.currentTimeMillis() + ".jpg");
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    // ⛔ שיתוף רק ב-createChooser · אין FLAG_ACTIVITY_NEW_TASK — ⚠️ המשתמש
                    // בוחר יעד בכל פעם, ⭐ והחזרה מביאה אותו חזרה למעטפת.
