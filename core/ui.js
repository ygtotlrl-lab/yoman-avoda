...
/* ═══ כפתור עסוק והשומר שבניתוב — מודול משותף ════════════════════════════
   ...
   ⚠️ בלי השבתה לחיצה שנייה על «שמור» בזמן שהראשונה באוויר שולחת בקשה
   שנייה, ⚠️ ובכתיבת משתמש היא יוצרת שורה כפולה או דורסת עדכון שטרם חזר.
...
/* ═══ שכבת המודאל — מודול משותף ═══════════════════════════════════════════
   ⛔ שכבת המודאל ושלושת העוזרים כאן ⛔ ולא ב-`index.html` — ⚠️ `esc` היא
      הגנת ה-HTML היחידה שיש, ⭐ וצורה שנייה שלה היא פרצה שאיש אינו מודד.
...
/*  ⛔ מיכל שאינו ב-DOM ⟵ **זעקה**, ⛔ ולא יציאה בשקט — ⚠️ מיכל שיושב
 *  בתוך מסך נמחק יחד איתו, ⭐ ו-`return` שקט נראה כ«הכפתור לא עשה
 *  כלום»: ⛔ והחיפוש הולך לדלגציה ולמטפל. ⛔ ו-`locked` מוסרת
...
/*  ⛔ `div` לכל הודעה, ⛔ ולא אלמנט קבוע אחד — ⚠️ באלמנט יחיד הודעה
 *  שנייה דורסת את הראשונה: ⛔ «נשמר» ו«שגיאת רשת» ברצף משאירים על המסך
 *  אחת בלבד, ⚠️ ומי שקורא את המסך אינו יודע שהייתה שנייה. ⛔ `kind` הוא שם מחלקה ⛔ ולא משך — ⚠️ ומי שאין לו ערך
