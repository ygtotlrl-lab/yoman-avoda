/* ═══ core/sync.js — ליבת הסנכרון ═══════════════════════════════════════
   ...
   השורות: «`pend` — ממתין לסנכרון» · «`merge` — ליבת המיזוג» ·
   «`ids` — מזהי רשומות» · «`rty` — ניסיון חוזר» ·
   «`pull` — מנגנון המשיכה» · «`push` — שכבת הדחיפה» ·
   «פעולה מגיבה מיד» · «משיכה מסוננת בשרת» · «חתימת סכימה» ·
   «`tombstones`» · «ברירת מחדל אינה ממוזגת» · «כל דחיפה נרשמת ביומן»
