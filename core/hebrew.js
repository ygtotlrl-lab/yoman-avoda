/* ═══ core/hebrew.js — מנוע התאריך העברי ════════════════════════════════
   ⭐ המנוע, הטבלה האריתמטית והנפילה-חזרה ביניהן, וצרכן התצוגה.
   ⛔ **והמודול מתקין את עצמו על `window`** — ⚠️ צרכניו קוראים לו בשם אחד,
      ⭐ והייבוא בדף הוא לתופעת הלוואי בלבד.
   ⛔ המודול זהה בית-לבית בכל ריפו שנושא אותו — ⚠️ ורק מי שיש בה צרכן
      תאריך נושאת אותו.
...
/* ═══ מנוע התאריך העברי — מודול משותף ═════════════════════════════════════
   ...
   ⛔ **ושלושת החלקים יושבים יחד** — ⚠️ חלקים פזורים נערכים אחד בלי
      השניים: ⛔ וכאן שלושתם בקובץ אחד. */
/*  ⛔ הלוח, הגימטריה והקריאה מ-`Intl` יושבים כאן ⛔ ולא ב-`index.html` —
 *  ⚠️ הם תשתית לכל דבר, ⛔ ושם שנושא תחילית של אפליקציה מסתיר שהם
 *  משותפים. */
...
/*  ⛔ אין לבדוק `d instanceof Date` ואין ליפול-חזרה ל«היום» —
 *  `instanceof` תלוי-realm: אובייקט תאריך שנוצר ב-realm אחר (iframe,
 *  רתמת `vm`) מחזיר `false`, ⛔ ונפילה-חזרה ל«היום» מחליפה אותו **בשקט**
 *  בתאריך של היום — ⚠️ תאריך שגוי שמוצג כאילו הוא נכון.
 *  ⭐ הבדיקה כאן היא על **החוזה** (`getTime` שמחזיר מספר) ואינה
 *  תלוית-realm; ⛔ וקלט שאינו עובר אותה מוחזר כ«לא תקף» ונרשם.
 *  ⚠️ קריאה **בלי ארגומנט** משמעה «היום», ורק ארגומנט שנמסר ואינו תאריך
 *  תקף נחשב שגיאה.                                                     */
 ...
 *  קריאה, ⭐ ומסך שמציג רשומות חוזר עליה פעם לכל רשומה: אלפי קריאות על
 ...
 *  לאותו יום.
 *  ⛔ קלט פגום אינו הופך ל«היום» — ⚠️ `jsDate || new Date()` מסתיר
 *  ארגומנט שגוי ומציג את תאריך היום כאילו הוא התאריך שנתבקש;
 *  ⭐ מחרוזת ריקה היא מה שכל הצרכנים יודעים לטפל בו.
 *  ⚠️ וקריאה **בלי ארגומנט** משמעה «היום». */
