 ...
 *
 *  ⚠️ **אינו שער מעבר:** מה שהשער נועל הוא **היעדר** הנפילה-חזרה —
...
const APP = {
  /*  ⭐ שכבת העימוד שהקריאה עוברת בה — ⛔ **אינה נגזרת**: שם פונקציה ב-`index.html`, ⚠️ ואין קובץ שמצהיר עליה */
  rowsGet: 'yaRowsGet',
  /*  ⭐ המפתחות שנקראים מטבלאות — ⛔ **אינם נגזרים**: שמות טבלאות בקוד, ⚠️ והסכימה נמדדת בשער אחר */
  keys: ['ya_entries', 'ya_archive'],
  /*  ⛔ ריצפת אתרי הקריאה — שני מפתחות × שלושה מסלולים (syncFromCloud · yaPullFromCloud ·
      _yaVerify) ועוד ספק החלון החם: ⚠️ **אינה נגזרת**, ⭐ היא מספר מסלולים במוצר. */
  minRowsGet: 5,
  /*  ⛔ שני המסלולים שנמדדים בטענה 1ג — ⚠️ שמם הוא מה שקושר את המשיכה לשער
      ה-`ok` שלה, ⛔ ובלעדיו הטענה מודדת דפוס ולא אתר: ⭐ שמות משתנים, ואינם נגזרים. */
 ...
 *  טענה משותפת שאבדה. */
const FLOOR = { shared: 0, app: 15, appWhy: 'מקור הקריאה מהטבלאות — מספר הטבלאות שהמסך מושך נבדל' };
...
/*  ⛔ והשורה **היחידה** שמותר לה לשבת בין ההמתנה לשער ה-`ok` היא שער
    ההקשר — ⚠️ הטענה היא שהמיזוג יושב **בתוך** `if (v.ok)`,
...
function env(total, mode, mutSrc) {
  ...
  const sb = {
    ...
    /*  ⛔ המוסד שהסביבה מחזירה בכל עמוד נרשם — ⚠️ הסגור נקרא
  ...
  /*  ⛔ העימוד עבר למודול המשותף — ⚠️ הסביבה טוענת אותו כמו כל
...
/*  ⛔⛔ דליפת העימוד — ⚠️ הנימוק המדוד: הסגור שמועבר לשכבת
...
/* ── 3. מוטציות ────────────────────────────────────────────────────────── */
console.log('— מוטציות —');
/*  ⛔ מוטציה **חיה** ולא סטטית — ⚠️ הסגור חוזר לקרוא את המוסד
