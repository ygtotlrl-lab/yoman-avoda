 ...
 *
 *  ⭐ **פרוטוקול תיעוד קבוע.** נמדד שרכיב
 *  שמוכרז «משותף» נסחף בלי אכיפה — ושאותו דבר בדיוק קרה
 *  לתיעוד עצמו: פרק «בדיקה» הועתק כלשונו לכל הריפו, כולל
 ...
 *       `SHARED`.
 *    ז. פסקה משותפת בשלושת הקבצים הנלווים אינה תואמת
 *       לחתימת ה-sha256 שלה, נמחקה, או נוספה בלי רישום קנוני.
 *    ה. הקובץ שונה מול `origin/main` אבל שורת «עודכן לאחרונה»
 *       זהה לזו שב-`origin/main` — כלומר לא קודמה. סעיף ג בודק את
 *       **צורת** השורה; זה בודק שהיא באמת התקדמה.
 *    ח. ערך של מפתח משותף ב-`manifest.json` נסחף או נמחק.
 *    ט. `CLAUDE.md` מחזיק יותר פרקי סבבים מ-`DOC_MAX_ROUNDS`,
...
/* ⚠️ מספר הסבב יכול לשאת סיומת עברית — «32ב», «35ג»: סבב-המשך
 *    נושא את מספר הסבב שהוא ממשיך ולא מספר חדש: ⚠️ בלי הסיומת סבב-המשך
 *    שרץ **באותו יום** של הסבב שהוא ממשיך אינו יכול לקדם את השורה כלל —
 *    התאריך זהה, והמספר נדחה. ⛔ הסיומת היא תו עברי אחד ולא טקסט חופשי —
 *    «(המשך)» היה מחזיר את הסחיפה שהפורמט הקבוע בא למנוע. */
...
/* ── ה. שורת העדכון קודמה בפועל מול origin/main ────────────────────────── */
/*  הפער שדווח: סעיף ג בודק שהשורה קיימת, בראש הקובץ ובפורמט
...
/* ── ה2. קידום `CACHE_NAME` מול origin/main ─────────────────────────────────
...
/* ── ו. שלד קבוע לשלושת קובצי ה-md הנלווים ──────────────────────────────────
 ...
 *  פרקים שלמים.
 *
 *  ⚠️ מה שנאכף כאן הוא **שלד ולא תוכן** — כותרות `##` נדרשות,
...
const MD_SKELETONS = [
  ...
  /* ⭐ `CONTEXT.md` מחזיק **לקוח · צורך · הסכימה וההרשאות**, ולכן
   ...
   *  ⛔ **ו«מצב נוכחי» ירד מהשלד** — ⚠️ הוא היה צילום מצב, כלומר
  ...
  /*  ⛔ ארבעה פרקים ירדו מהשלד — «Why WebView», «גשר», «למה אין
  ...
  for (const [re, label] of spec.need) {
    ...
    if (found < 0) {
      ...
      fail(`${spec.file}: הפרק «${label}» ` +
        ...
        'שלד הקובץ זהה בכל הריפו');
    } else at = found + 1;
  }
  if (ok) pass(`${spec.file} — ${spec.need.length} פרקי השלד קיימים ובסדר`);
}

/* ── ז. פסקאות משותפות בשלושת הקבצים הנלווים ────────────────────────────────
 *  ⚠️ **סעיף ו אוכף שלד, ולא תוכן** — כותרות `##` בסדר יחסי, וזה כל מה
 *  שהוא רואה. ⚠️ המחיר נמדד: ל-`android/README.md` של gius חסרה
 ...
 *  בהשוואת כותרות ולא בספירת פסקאות — שתיהן עוברות על פסקה
 ...
 *  בציר אחר.                                                             */
/*  ⛔ הרשימה כתובה **בסדר הופעתם בקובץ** — ⚠️ סעיף יב אוכף
...
const CANON_MD = [
  ...
  ['android/README.md',  'android-shell-split',   'ff2af369bd18f08b'],
...
/* ── ח. manifest.json — **ערכי** המפתחות המשותפים ───────────────────────────
   ...
   ⚠️ **וזה אותו לקח בציר אחר** — שם הושוו **שמות** הכותרות
...
const CANON_APP_ID = APP_SCOPES;
{
  ...
  if (!fs.existsSync(file)) {
    ...
    if (mf) {
       ...
       *  `icon-maskable-512.png` — שקיים בכולן — לא הופיע
...
/* ────── ט. תקציב התיעוד — חלון של שני סבבים ותקרה של 700 שורות ─────────────
   ⚠️ **תשעה סשנים נחנקו בשבוע אחד**, ונמדד ש-`CLAUDE.md` תפח מ-1,782
   שורות (אחרי הגיזום) ל-4,447 — ⛔ מפני שכל סבב מוסיף פרק ואף
   ...
   ⭐ **החלון הודק משישה לשניים** — git מחזיק את ההיסטוריה,
...
/*  ⛔ תקרות שלושת הקבצים הנלווים, ⛔ ותקרה נפרדת לכל חלק —
 ...
 *  ⛔ והתקרות הן **מספרים עגולים שנקבעו בהכרעה** — ⚠️ ולא המצב
...
/*  ⚠️ `[משותף, פרטי]` — ⛔ שני הרפים נאכפים בנפרד, ⛔ ולא סכומם.
...
/*  ⛔ סכום שני החלקים = תקרת הקובץ — ⚠️ שתי תקרות שסכומן נמוך
...
const ROUND_H2 = /^##\s+(?:⭐\s+)?סבב\s/;
{
   ...
   *  ריקות בסופו — שורה ריקה אינה תוכן, ואין להעניש עליה. */
  let openRound = -1;
...
/* ── ⭐ תקרת התוכן הפרטי-הקבוע — 900 שורות ───────────────────────────────────
...
const DOC_MAX_PRIVATE = 300;
{
  ...
  /*  ⛔ הפרטי הוא כל מה שאינו בבלוק משותף — ⚠️ פרקי הסבבים
  ...
  for (const [f, cap] of Object.entries(MD_MAX)) {
    ...
    /*  ⛔ הפיצול נמדד באותה מסכת גדרות-קוד — ⚠️ סמן SHARED
...
/* ────── י. תוכן שני חלקי CLAUDE.md — כל חלק לפי הגדרתו ─────────────────────
   ...
   הקובץ בפועל. */
{
  ...
  /*  ⛔ בחלק המשותף אין כותרת `##` כלל — ⚠️ הבלוקים נפתחים
...
/* ────── יב. סדר הבלוקים (⛔ פרקי הכללים ירדו) ───────────────────────────────
   ...
   ההוראה, ⛔ וניסוחה נמדד ב-`sha256` של הבלוק היחיד. */
{
  ...
  /*  ⛔ **ואין `###` בתוך הבלוק המשותף** — ⚠️ הבלוק מחזיק את
