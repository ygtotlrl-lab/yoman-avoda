 ...
 *  — ⚠️ ביומן עברו שני אתרי היצירה ל-`newClientId()` יחד עם שלושת
...
const START = '/* ═══ מזהי רשומות — מודול משותף';
...
console.log('· ' + FACTS.slug + ' — מודול מזהי הרשומות');
...
if (RUN_MUT) {
  mutStage();
/* ── 3 · מוטציות ───────────────────────────────────────────────────────── */
/*  ⛔ הבודק מיובא ⛔ ואינו מורץ כתהליך — ⚠️ הטענה כאן היא על
...
console.log(failed ? `\n✗ מזהים — ${failed} טענות נכשלו`
                   : `\n✓ מזהים — ${passed} טענות עברו`);
