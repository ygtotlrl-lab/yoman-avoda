#!/usr/bin/env node
/*  test_readonly.mjs — ⛔ שער קורא בלבד, והנעילה המבנית.
 *
...
function copyRepo() {
  /*  ⛔ עותק לכל תרחיש, ⛔ ובכוונה — ⚠️ נמדדו **שישה** בהרצה
 ...
 *  ⛔ `GATE_INNER` נדרש כאן — ⚠️ בלעדיו הריצה הפנימית פותחת עותק משלה. */
/*  ⛔ הריצה הפנימית אינה יורשת `GATE_MUT` — ⚠️ הטענה כאן היא
...
function runGate(dir, tool, extraEnv) {
  const env = { ...process.env, GATE_INNER: '1', ...extraEnv };
  delete env.GATE_MUT;
...
/*  ⭐ זו גם הבקרה החיובית של `check-structure` ושל `check-js`:
 *  שניהם רצים כאן, בתוך הסט. ⛔ אין ריצה נוספת שלהם לבדם —
...
/* ── ה. הנעילה המבנית — `check-structure` על עץ מומט ───────────────────── */
{
  ...
  /*  ⛔ תוכן ארבע התיקיות — עד אז `android/`, `migrations/`,
  ...
  /*  ⛔ שם השער נגזר מהנושא — `test_round52_pendflush` לא אמר
...
/*  ⛔ בשלבי ה-`node --check` בלבד — ⚠️ ריצה מלאה כאן הייתה
 ...
 *  מוקטנת גם תקרת הסט נחצית, ⛔ וקוד היציאה היה עונה על שאלה אחרת. */
{
  ...
  const run = (gates) => {
    ...
    const env = { ...process.env, GATE_INNER: '1' };
    delete env.GATE_MUT;
...
console.log(fail ? `\n✗ קורא בלבד + נעילה מבנית — ${fail} נכשלו, ${pass} עברו`
                 : `\n✓ קורא בלבד + נעילה מבנית — ${pass} טענות עברו`);
