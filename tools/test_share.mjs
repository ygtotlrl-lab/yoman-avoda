 ...
 *  ⚠️ ונמדד אפס קוד שיתוף בשלוש האחרות. ⭐ הכלל עצמו כן משותף בכולן.
...
const APP = {
  ...
  /*  ⚠️ המעטפת הפר-אפליקציתית — ⛔ ולא הליבה: `ShellActivity` זהה
      בית-לבית בכולן ואין בה שיתוף, והיא נחתמת ב-`coreSha`. */
...
/*  ⛔ אין לרשום כאן שורה (סבב 72) — ⚠️ שורת גשר השיתוף נאכפת ב-MATRIX
 ...
 *  טענה משותפת שאבדה. */
const FLOOR = { shared: 0, app: 5, appWhy: 'מסלול השיתוף — רק היומן מייצא קובץ החוצה' };
