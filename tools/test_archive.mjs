 ...
 *
 *  **מה נאכף:** הפונקציות נחתכות מ-`index.html` לפי שמן ורצות ב-`vm` —
 *  ⛔ מוטציה בקוד האמיתי מפילה טענה; ⚠️ והנתונים הם **מדידה מהמסד החי**
 *  ⛔ ולא נתונים שהומצאו כדי להתאים לקוד.
 *
 *  **הנימוק המדוד:** מסלול הארכוב רץ **פעם ביום** — ⚠️ באג בו מתגלה
 *  למחרת, ⛔ ואז הנתון כבר נכתב.
 *
 *  **מה יישבר בלעדיו:** ⛔ סנאפשוט שנבנה בשני מסלולים שונים ממלא שדה
 *  באחד ומשמיט אותו בשני, ⚠️ והמיזוג מכריע לפי מה שחסר.
 *
 *  **מה אינו נאכף כאן:** ⛔ מסלול ה-PDF והייצוא — ⚠️ הוא תצוגה, ⭐ והשער
 *  מודד את **מקור** הרשומות.
 *
 *  ⚠️ פרטי לאפליקציה הזו. ⛔ אין ליישר אותו מריפו אחר — אין לארכיון הזה
 *  מקבילה באף אחת מהאחיות. ⛔ אין שעון ואין `setTimeout` בקובץ — כל טענה
 *  נמדדת על **אירוע**, ⛔ ולא על המתנה.
 */
 ...
 *  טענה משותפת שאבדה. */
const FLOOR = { shared: 0, app: 137, appWhy: 'הארכיון — תצוגה ומקור הרשומות האוטומטיות, יכולת שקיימת ביומן בלבד' };
...
if (!process.env.RD67_MUT) {
  ...
  mutStage();
  if (!RUN_MUT) {
    console.log('\n⏭ test_archive: המוטציות רצות ברמה המלאה (--full) — ⛔ ואינן נמדדות כאן');
