...
const APP = {
  ...
  toolsExtra: {
    ...
    '_prune-lessons.md': 'לקחי פרקי הסבבים שנגזמו (הפרקים עצמם נמחקו)',
  },
  keystore: 'yoman.keystore',
  androidExtra: {
    /* ⭐ גשר השיתוף קיים ביומן בלבד (שורת גשר השיתוף במטריצה) — ה-`FileProvider`
       שבמניפסט מצביע על הקובץ הזה, ⛔ ומחיקתו — קריסה בזמן ריצה.
       ⛔ **וזהו הקובץ ה-21 מול 20 בשלוש האחרות** — ⚠️ הבדל מכוון, ⛔ ואין
  ...
  toolsDirs: {
    'fixtures': 'פיקסטורות לבדיקות הסבבים (הארכיון)',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
const FLOOR = { shared: 14, app: 2, appWhy: 'גשר השיתוף ותיקיית הפיקסטורות — חריגות מבנה שקיימות ביומן בלבד' };
/* ⚠️ סוף פר-אפליקציה */
