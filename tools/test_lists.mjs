 ...
 *  ⛔ שם, מרכאות ותוכן **אינם** מנורמלים — נרמול שלהם היה הופך
...
function capsEnds(src) {
  ...
  while ((m = re.exec(src))) {
    ...
    /*  ⛔ בלוק CSS מנוכה בגזירה — ⚠️ `check-comments` סורק את
 ...
 *   — שינוי פורמט שמפיל את הגזירה היה הופך את הבדיקה
 ...
 *   — בדיוק מה שרשימת-היתר שהתיישנה עושה ב-`check-structure.mjs`. */
...
/*  6 — מוטציות. ⛔ רצות על עותק בזיכרון ולא על העץ — מוטציה שנכתבת לקובץ האמיתי מותירה את הריפו שבור אם
 *  התהליך נהרג באמצע. */
