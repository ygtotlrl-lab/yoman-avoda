#!/usr/bin/env node
/*  test_stage_b.mjs — שכבת השורות בענן ונוסחת הזהות.
 *
 *  **מה נאכף:** הפונקציות נחתכות מ-`index.html` ורצות ב-`vm` מעל לקוח
 *  מדומה — ⛔ מוטציה בקוד האמיתי מפילה טענה; ⛔ ונוסחת הזהות נבדקת מול
 *  **מימוש עצמאי של ה-SQL** שבמיגרציה.
 *
 *  **הנימוק המדוד:** שני צדדים שמחשבים זהות אחרת יוצרים **שתי שורות
 *  לאותה רשומה** — ⚠️ וזו התקלה היחידה כאן שאין ממנה חזרה שקטה.
 *
 *  **מה יישבר בלעדיו:** ⛔ רשומה כפולה בענן אינה שגיאה — ⚠️ היא פשוט
 *  מופיעה פעמיים, ⭐ והמיזוג מכריע ביניהן בלי מפתח.
 *
 *  **מה אינו נאכף כאן:** ⛔ הרצת המיגרציה עצמה — ⚠️ הבדיקה קוראת את ה-SQL
 *  כטקסט, ⭐ ומצב ההרצה נמדד מול המסד.
 *
 *  ⚠️ פרטי לאפליקציה הזו. ⛔ אין ליישר אותו מריפו אחר — שכבת השורות הזו
 *  אינה קיימת באף אחת מהאחיות.
 */
 ...
 *  טענה משותפת שאבדה. */
const FLOOR = { shared: 0, app: 63, appWhy: 'שכבת השורות בענן ונוסחת הזהות — היומן הוא הצרכן הגדול בדומיין' };
...
if (!process.env.RD67_MUT) {
  ...
  mutStage();
  if (!RUN_MUT) {
    console.log('\n⏭ test_stage_b: המוטציות רצות ברמה המלאה (--full) — ⛔ ואינן נמדדות כאן');
