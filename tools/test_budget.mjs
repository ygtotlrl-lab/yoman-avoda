 ...
 * ⛔ הקובץ זהה בית-לבית בכל הריפו — אין בו בלוק `APP`.
 */
...
/* ⚠️ אותה מסכת גדרות-קוד של סעיף א — ⛔ כותרת שבתוך ``` אינה פרק. */
...
/* ⭐ החלק הפרטי-הקבוע — כל מה שאינו בלוק `SHARED`.
   ⚠️ אותה מסכת גדרות-קוד: ⛔ סמן `SHARED` שבתוך דוגמה אינו בלוק אמיתי.
   ⛔ **ופרקי הסבבים נספרים בו** — ⚠️ עד הסבב הזה הם הוחרגו כאן
...
/*  ⛔ המוטציה **חותמת מחדש** את הבלוקים המשותפים בעותק —
...
/*  ⛔ עותק אחד לשער, ⛔ ולא עותק לכל מוטציה — ⚠️ נמדד שהשער
...
/* ⭐⭐ **פינוי מקום לפני כל מוטציה שמוסיפה שורות — ⛔ ולא ויתור עליה
  .** ⚠️ **הבעיה שנמדדה:** ככל שהקובץ מתקרב לתקרה הגלובלית, כל
...
/* ⚠️ המוטציה מוסיפה **מספיק** פרקים כדי לחצות את החלון, ולא פרק אחד
    — ⛔ ריפו שגזם מתחת לתקרה היה עובר מוטציה של פרק בודד, כלומר
...
/* ⚠️ הפינוי נדרש כאן מאותה סיבה בדיוק שבמוטציית החלק הפרטי שלמטה —
...
/* ⚠️ הריפוד הוא כותרת `###` ⛔ ולא `##` — ⚠️ כותרת `##` בחלק
...
/*  ⛔ ופינוי מגוף פרקי הסבבים אינו עוזר כאן — ⚠️ מרגע שהם
...
/*  ⭐ מוטציית-נגד — ⛔ שינוי שחייב **לעבור**: שלוש שורות פרטיות
...
/* ── ⛔ אורך פרק הסבב — ⭐ ולא רק מספר הפרקים ─────────────────────────────────
   ⚠️ בעבר נספרו הפרקים ולא נמדד אורכם, ⛔ ולכן שני פרקים בני 75 ו-108
...
/*  ⛔ ומוטציה על החלק המשותף — ⚠️ התקרה המשותפת היא תקרה
    **נפרדת**, ותוספת בתוך בלוק משותף אינה נספרת בפרטי. ⛔ והריפוד נגזר
    מהנמדד ואינו קבוע: קבוע שנכתב פעם אחת חוצה עם הזמן גם את
...
console.log(fail ? `\n✗ תקציב תיעוד — ${fail} נכשלו, ${pass} עברו`
                 : `\n✓ תקציב תיעוד — ${pass} טענות עברו`);
