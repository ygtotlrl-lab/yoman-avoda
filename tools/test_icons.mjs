...
/* ⛔ אין הפניה ל-design/ מקובץ מוגש — זו הורדה במגה-בייט שהמשתמש
   משלם עליה לשום צורך. */
...
console.log(fail ? `\n✗ שכבת האייקונים — ${fail} נכשלו, ${pass} עברו`
                 : `\n✓ שכבת האייקונים — ${pass} טענות עברו`);
process.exit(fail ? 1 : 0);
