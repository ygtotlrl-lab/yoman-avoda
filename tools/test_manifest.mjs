...
/*  ⛔ הקובץ הזה אינו אוכף שורה בטבלת התשתית — ⚠️ הצהרה ריקה
 *  ולא היעדר: ⛔ שער בלי הצהרה אינו נבדל משער שההצהרה שלו נשמטה. */
export const ROWS = [1, 148];
...
const ok  = (n, m) => { RAN++; pass++; console.log(`  ok   ${n} · ${m}`); };
const bad = (n, m) => { RAN++; fail++; console.log(`  FAIL ${n} · ${m}`); };
...
/*  ⛔⛔ `id` ו-`scope` **ייחודיים** — ⚠️ הרישום מחזיק את כל האפליקציות,
    ⭐ מפני ששער שרואה ערך אחד אינו יכול למדוד ייחודיות: ⛔ כל הערכים
    היו `"./"`, שנפתר לאותה כתובת מוחלטת בכולן. */
const APP_ID = APP_SCOPES;
/* ⛔ אלה נשארים פרטיים ואין לאכוף את ערכם — זהות חזותית
   פר-אפליקציה. */
...
/*  ⛔ **הנימוק המדוד:** השורה הצהירה «`id` זהה בכולן» וה-probe בדק
 *  שהשדה **קיים** ושערכו הוא הערך המוצהר — ⚠️ כולן נשאו `"./"`, שנפתר
 *  לאותה כתובת מוחלטת, ⛔ וכרום ראה בהן אפליקציה אחת. ⭐ הטענה כאן היא
 *  על **ייחודיות בין כל הערכים**, ⛔ ולא על ערך יחיד.
...
t(n++, new Set(idVals).size === APP_ID.length,
  `⭐ ו-${APP_ID.length} המזהים שונים זה מזה — נמדדו ${new Set(idVals).size} ערכים שונים והצפוי ${APP_ID.length}`);
...
/*  ⛔ הזוג נמדד **במרשם** ⛔ ולא בטקסט של הבודק — ⚠️ שני עותקים של אותה
 *  רשימה הם שני מקומות שיכולים להיבדל, ⭐ ומשיש מרשם אחד הנמדד הוא
 *  שהבודק קורא אותו ⛔ ושהשם חי בו: ⚠️ שם שיורד מהמרשם מפיל כאן. */
const peersSrc = fs.readFileSync(path.join(HERE, 'peers.mjs'), 'utf8');
const docsDerives = /from '\.\/peers\.mjs'/.test(docs) && /CANON_APP_ID = APP_SCOPES;/.test(docs);
for (const [a, v] of APP_ID)
  t(n++, docsDerives && peersSrc.includes(`'${a}'`),
...
/*  ⛔ עותק אחד לשער, ⛔ ולא עותק לכל מוטציה — ⚠️ נמדד שהשער פתח
    **שמונה** עותקי עץ בהרצה אחת: ⭐ כל מוטציה כאן כותבת `manifest.json`
    בלבד, ⛔ ולכן שחזורו מהעץ האמיתי מחזיר את העותק למצבו הנקי. */
/*  ⛔ כותב על עותק — ⚠️ הבודקים קוראים את המניפסט מהדיסק, ⛔ ואין דרך למסור להם אובייקט. */
...
t(n++, runDocsOn(mutScope) === false,
  '⛔ מוטציה: החזרת `scope` ל-`./` **מפילה** — הערך שאיחד את כולן');
...
console.log(`\n${fail ? '❌' : '✓'} manifest — ${pass} טענות עברו, ${fail} נכשלו`);
