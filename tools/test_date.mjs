 ...
 *  טענה משותפת שאבדה. */
const FLOOR = { shared: 0, app: 32, appWhy: 'מנוע התאריך העברי — צרכני תאריך יש ביומן ובהנהלה בלבד' };
...
/* ── 1. טענות סטטיות ───────────────────────────────────────────────────── */
/*  ⚠️ הטענות נמדדות על **קוד** ולא על הערות — ההערה שמסבירה
...
/*  ⛔ הטענה היא על **הערך** ולא על קיום השם — ⚠️ טבלת החודשים
...
console.log(failed ? `\n✗ התאריך העברי — ${failed} טענות נכשלו`
                   : `\n✓ התאריך העברי — כל הטענות עברו`);
