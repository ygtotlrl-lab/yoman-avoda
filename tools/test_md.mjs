 ...
 *  בכל הריפו. ⛔ והמוטציות רצות **בתהליך אחד** — ⚠️ תהליך חדש לכל מוטציה
...
const ok = (m) => { RAN++; n++; console.log(`  ok   ${m}`); };
const no = (m) => { RAN++; n++; bad++; console.error(`  FAIL ${m}`); };
const t  = (c, m) => (c ? ok(m) : no(m));

/* שלושת הקבצים והפרק שנמחק בכל מוטציה — הכותרת הראשונה שהשלד דורש. */
const CASES = [
  ['README.md',         /^##\s+מסכים\s*$/m,                 'מסכים'],
  /*  ⛔ «מצב נוכחי» ירד מהשלד — ⚠️ הוא היה צילום מצב, כלומר
...
console.log('· שלד שלושת קובצי ה-md');
...
/*  ⛔ כל תיקייה זמנית נרשמת ונמחקת בסוף — ⚠️ נמדד: הרתמה
 *  יצרה כ-30 תיקיות בכל הרצה ולא מחקה אחת, ⛔ ואלפי עותקי עץ מילאו את
 *  הדיסק עד ENOSPC באמצע הרצת שער. */
/*  ⛔ כל גופו של השער הזה הוא רתמה על עותקי עץ — ⚠️ הבקרה
...
/* 4ב — שינוי בית אחד בתוך פסקה משותפת מפיל */
{
  ...
  /*  ⛔ שם המשתנה אינו `src` — ⚠️ `src` שמור למקור האפליקציה,
...
/* ────── 5 — ארבעת הפרקים שהוכרעו כמשותפים ביישור תיעוד האנדרואיד ───────────
 *  ⚠️ נמדד ששלושה נושאים בתיעוד האנדרואיד חיו בשלוש כותרות שונות
 *  לאותו דבר, ושפרק «אייקונים» קיים בריפו אחד בלבד אף שהמבנה שהוא מתאר
 *  זהה בכולן (עשרה `mipmap` ו-XML אדפטיבי אחד — נמדד). ארבעת הפרקים
...
console.log('· הפרקים המשותפים של תיעוד האנדרואיד');
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
console.log('· שכבת האייקונים במניפסט');
...
console.log(bad ? `\n❌ שלושת קובצי ה-md — ${bad} מתוך ${n} נכשלו` : `\n✓ שלושת קובצי ה-md — ${n} טענות עברו`);
process.exit(bad ? 1 : 0);
