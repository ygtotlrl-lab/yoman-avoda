...
-- ⚠️ קובץ **הגירה חד-פעמי, היסטורי** — ההגירה כבר בוצעה.
-- ============================================================
-- ⛔ **זהו אינו קובץ ההתקנה ואינו מקור האמת לסכימה** (סבב 28) — לשם כך יש
--    `migrations/000_initial_schema.sql`. ⛔ אין להריץ את הקובץ הזה בהתקנה
--    חדשה: הוא מסתיים ב-`insert … select … from public.kv`, כלומר הוא מעתיק
--    מטבלת ה-`kv` שקדמה לרב-דיירות — טבלה ששייכת היום ל-hanhala-ruchanit
--    ואינה קיימת בפרויקט חדש. הרצה חוזרת נכשלת שם, ובפרויקט המשותף היא
--    הייתה שופכת מפתחות זרים ל-`kv_rishon`.
-- ⚠️ הוא נשאר בריפו כתיעוד ההגירה שבוצעה, ולא כהוראת הפעלה.
-- ============================================================
